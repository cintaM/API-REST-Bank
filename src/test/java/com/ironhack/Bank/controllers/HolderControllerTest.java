...
public class HolderControllerTest {
    ...
    @Test
    void post_holder_isCreated() throws Exception {
        holder1 = holdersRepository.save(new Holders("Daniel", LocalDate.of(1985, 6, 26), new PrimaryAddress("ere", 2, "Barcelona"),
                "ijil@gjdj.com"));

        String body = objectMapper.writeValueAsString(holder1);
        System.out.println(body);

        MvcResult mvcResult =mockMvc.perform(post("/holder/add").content(body).contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isCreated()).andReturn();

        assertTrue(holdersRepository.findById(2L).isPresent());

    }
  @Test
    void get_All_OK() throws Exception {
        MvcResult mvcResult = mockMvc.perform(get("/holder/getall")).andExpect(status().isOk()).andReturn();
    ...
    void get_ById_OK() throws Exception {
        MvcResult mvcResult = mockMvc.perform(get("/holder/getone/1")).andExpect(status().isOk()).andReturn();
        assertTrue(mvcResult.getResponse().getContentAsString().contains("Maria"));
